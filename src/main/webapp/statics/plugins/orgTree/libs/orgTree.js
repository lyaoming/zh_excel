...
; (function($, window, document, undefined) {
    ...
    oTree.prototype = {
        dialog: function() {
            ...
            _this.$element.on('click', '.js_show_party_selector',
                function() {
        ...
        loadAll: function(ele, data) {
            ...
            function addDiyDom(treeId, treeNode) {
                ...
                pObj.find('.jstree-wholerow').eq(0).bind('click',
                    function() {
        ...
        searchFuc: function(data) {
            ...
            var sdom = '<div class="multiPickerDlg_search_wrapper">' + '<div class="multiPickerDlg_search">' + '<span class="ww_searchInput">' + '<span class="ww_commonImg ww_commonImg_Search ww_searchInput_icon"href="javascript:;"></span>' + '<span class="ww_commonImg ww_commonImg_ClearText ww_searchInput_delete" id="clearMemberSearchInput"></span>' + '<input type="text"id="memberSearchInput"class="qui_inputText ww_inputText ww_searchInput_text" placeholder="搜索部门"></span>' + '</div>' + '<div id="searchResult"class="ww_searchResult"style="display: none">' + '<div id="search_member_list_title"class="ww_searchResult_title ww_searchResult_title_First">搜索结果</div>' + '<div class="search_member_none">无搜索结果...</div>' + '<ul id="search_member_list"></ul>' + '</div>' + '</div>';
            ...
            $input.on('keydown',
                ...
                function(event) {
                    ...
                    if (inputValue) {
                        ...
                        $('.ww_searchInput_delete').on('click',
                            function() {
                        ...
                        $('#search_member_list li').on('click',
                            function() {
        ...
        getValue: function() {
            ...
            var filterJson = [];
            for (var i = 0; i < savedVal.length; i++) { (function(i) {
        ...
        outRepair: function() {
            var _this = this;
            $(document).on('click', '.js_disselect_party',
                function() {
                    $(this).parent().remove();
        ...
        renderOut: function(arr) {
            ...
            if (arr.length > 0) {
                arr.map(function(ele, index) {
                    perVal += ele.id + ';';
                    wrap += '<div class="ww_groupSelBtn_item">' + '<span class="ww_commonImg ww_commonImg_TreeMenuThumb"></span>' + '<span class="ww_groupSelBtn_item_text">' + ele.name + '</span>' + '<span class="ww_commonImg ww_commonImg_GroupDelSel js_disselect_party">x</span>' + '<input type="hidden" class="parydata" name="partyid" data-name="' + ele.name + '" data-id="' + ele.id + '" data-crumbs="' + ele.crumbs + '" value="' + ele.id + '">' + '</div>';
                });
                $('.js_show_party_selector').html('修改');
            } else {
                $('.js_show_party_selector').html('添加');
        ...
        renderNode: function(arr) {
            ...
            arr.map(function(item, index) {
                liHtml = '<li title=' + item.crumbs + ' data-name=' + item.name + ' data-id=' + item.id + ' class="ww_menuDialog_DualCols_colRight_cnt_item token-input-token js_list_item">' + '<span class="ww_commonImg icon icon_folder_blue"></span>' + '<span class="ww_treeMenu_item_text" title=' + item.crumbs + '>' + item.name + '</span>' + '<a href="javascript:" class="ww_menuDialog_DualCols_colRight_cnt_item_delete"><span class="ww_commonImg ww_commonImg_DeleteItem js_delete"></span></a>' + '</li>';
